...
- **浏览器自动启动** — 未检测到 Chrome debug 端口时，自动启动系统 Chrome（含 `--no-first-run`，避免卡在首次运行引导）
- **环境自检** — `boss_diagnose` 工具自动诊断 FRE 劫持/风控清页/登录态/代理问题并给出修复指引
- **YAML 配置** — 公司信息、JD、搜索关键词、筛选条件全部可配置

> 遇到浏览器/风控问题：先查 [docs/TROUBLESHOOTING.md](../docs/TROUBLESHOOTING.md)（症状→根因→已验证解法速查表）。
